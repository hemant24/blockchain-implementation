...
const getTransactionId = (transaction: Transaction): string => {
    ...
    .reduce((a, b) => a + b, '');  // It is initial value if array is empty initial value is used, https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/Reduce
...
const findUnspentTxOut = (transactinId : string, index: number, aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut=> {
    return aUnspentTxOuts.find((uTxO) => uTxO.txOutId == transactinId && uTxO.txOutIndex == index)
}
const signTxIn = (transaction: Transaction, txInIndex: number,
    ...
    const uTxO: UnspentTxOut = findUnspentTxOut(txIn.txOutId, txIn.txOutIndex, aUnspentTxOuts);
    ...
    if(uTxO.address !== getPublicKey(privateKey)){
...
const updateUnspentTxOuts = (newTransactions: Transaction[], aUnspentTxOuts : UnspentTxOut[]): UnspentTxOut[] => {
    ...
    .map((tx : Transaction) => {
        return tx.txOuts.map((txOut : TxOut, index: number) => new UnspentTxOut(tx.id, index, txOut.address, txOut.amount));
    }).reduce((a,b) => a.concat(b) , [])


    const consumedTxOuts: UnspentTxOut[] = newTransactions
    .map((tx : Transaction) => {
        return tx.txIns.map((txIn : TxIn, index: number) => new UnspentTxOut(txIn.txOutId, txIn.txOutIndex, '', 0));
    }).reduce((a,b) => a.concat(b) , [])

    const resultingUnspentTxOuts: UnspentTxOut[] = aUnspentTxOuts
...
const validateTransaction = (transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
    ...
    .map((txIn: TxIn) => validateTxIn(txIn, transaction, aUnspentTxOuts) )
    .reduce((a, b) => a && b, true)
